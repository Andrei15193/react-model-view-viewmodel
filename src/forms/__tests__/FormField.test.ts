...
describe('FormField', (): void => {
    ...
    it('creating a field with validators initializes it', (): void => {
        ...
        expect(field.initialValue)
            .toBeNull();
        expect(field.validation.validators.length)
            .toBe(1);
        expect(field.validation.triggers.size)
            .toBe(0);
    });
    ...
    it('creating a field with validation triggers initializes it', (): void => {
        ...
        expect(field.validation.triggers.size)
            .toBe(2);
        expect(Array.from(field.validation.triggers)
            .some((trigger) => trigger instanceof CollectionChangedValidationTrigger))
            .toBeTruthy();
        expect(Array.from(field.validation.triggers)
            .some((trigger) => trigger instanceof CollectionReorderedValidationTrigger))
            .toBeTruthy();
    });
